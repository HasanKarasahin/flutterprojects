...
class BooksViewModel extends ChangeNotifier {
  ...
  String _collectionPath = 'Settings';
  ...
  Stream<List<Book>> getBookList() {
    ...
    Stream<List<Book>> streamListBook =
        streamListDocument.map((listOfDocSnap) => listOfDocSnap
            .map((docSnap) => Book.fromMap({
                  "id": docSnap.id,
                  "bookName": docSnap["authorName"],
                  "authorName": docSnap["authorName"],
                  "publishDate": docSnap["authorName"],
                }))
            .toList());
